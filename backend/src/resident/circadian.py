...
class CircadianEvidence:
    ...
    local_hour: float             # 0..24 hour-of-day in the host's local timezone
...
class CircadianStateMachine:
    ...
    def __init__(
        ...
        *,
        ...
        tz: tzinfo | None = None,  # day/night clock reference zone; None = host-local
    ):
        self.store = store
        self.params = params or CircadianParams()
        # the day/night clock's reference zone (ADR-0023): None renders the
        # hour-of-day in the host's local timezone; a fixed zone (the
        # deterministic sims pin UTC) makes it host-independent.
        self.tz = tz
        # current state: reconstructed-from-log > ACTIVE (fresh mind)
    ...
    def assemble_evidence(
        store: EventStore, now_iso: str, current: str, tz: tzinfo | None = None
    ) -> CircadianEvidence:
        """Derive the decision evidence from the store + reference ``now``.

        ``tz`` is the day/night clock's reference zone (ADR-0023): ``None`` (the
        default) renders the hour-of-day in the host's local timezone; a fixed
        zone (the deterministic sims/tests pin ``timezone.utc``) makes it
        host-independent. The since_* deltas are absolute durations and ignore
        ``tz``.
        """
        p = CircadianParams()  # thresholds only; the machine's own params are applied in decide
        now = _parse_iso(now_iso)
        # The day/night clock renders the hour-of-day in ``tz`` (the host's local
        # wall clock by default), not the raw UTC store instant — a resident's
        # day must follow the machine's local time. A naive `now` is read as UTC.
        # Only the hour-of-day (hence the night/wake windows) is timezone-local;
        # the since_* deltas below still use the absolute instant `now`.
        if now is not None and now.tzinfo is None:
            now = now.replace(tzinfo=timezone.utc)
        _local = now.astimezone(tz) if now else datetime.now().astimezone(tz)
        hour = _local.hour + _local.minute / 60.0
...
class CircadianOrchestrator:
    ...
    **Not a cron.** The beat *samples* time (a cadence), but *what happens* —
    ...
    def __init__(
        ...
        agency=None,  # v1.0 (ADR-0018): AgencyService (optional; routes Mind-proposed intents)
        tz: tzinfo | None = None,  # day/night clock zone for a self-created machine (ADR-0023)
    ):
        self.store = store
        self.mind = mind
        self.sleep_engine = sleep_engine
        self.reentry = reentry
        self.machine = machine or CircadianStateMachine(store, params=params, tz=tz)
        self.params = params or self.machine.params
        self.world_engine = world_engine  # Phase 6 (ADR-0008): opens on awake beats
        # v1.0 (ADR-0018, Phase B): the Agency, if mounted. A Mind-proposed
        # intent is ROUTED here (as a background task) — never acted on inside
        # the life loop. None in every pre-v1.0 construction (byte-identical).
        self.agency = agency
        self._pending_actions: set = set()  # in-flight action tasks (kept alive)
        self.beats = 0
        self.wakes = 0
    ...
    async def beat(self, now_iso: str) -> dict:
        ...
        """
        self.beats += 1
        ev = self.machine.assemble_evidence(self.store, now_iso, self.machine.state, tz=self.machine.tz)
    ...
    def _route_intent(self, res: dict) -> None:
        """v1.0 (ADR-0018, Phase B): if this wake's reflection PROPOSED an intent,
        hand it to the Agency — as a background task, so a life beat is never
        blocked by an action. The Agency's deterministic policy then decides
        whether the intent may run: the Mind proposed an opportunity, it did not
        grant itself permission (principle 1). With no Agency mounted, or
        agency_propose off (so ``proposed_intent`` is always None), this is a
        no-op — sealed/flag-off behaviour is byte-identical.
        """
        intent = res.get("proposed_intent")
        if self.agency is None or not intent or not intent.get("text"):
            return
        task = asyncio.create_task(self._run_proposed_intent(
            text=str(intent["text"]),
            capability=intent.get("capability"),
            caused_by=[res["event_id"]] if res.get("event_id") else None,
            thread_id=res.get("thread_id"),
        ))
        self._pending_actions.add(task)
        task.add_done_callback(self._pending_actions.discard)

    async def _run_proposed_intent(self, *, text: str, capability, caused_by,
                                   thread_id) -> dict:
        """Wrap the Agency intake so a background task can never take down the
        life: any error is contained and returned as a plain summary, not raised.
        (The Agency itself always records a terminal outcome; this guard is for
        the unexpected.)"""
        try:
            return await self.agency.intake(
                text=text, capability=capability, source="mind",
                caused_by=caused_by, thread_id=thread_id,
            )
        except Exception as exc:  # noqa: BLE001 - a background action must not crash the loop
