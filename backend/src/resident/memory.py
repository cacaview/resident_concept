...
class MemoryRetrieval:
    ...
    def topic_concentration(self, n_recent: int = 40) -> float:
        ...
        signal (see topic_distribution) but keeps this coarse read for the
        route policy's open-endedness guard.
    ...
    def topic_distribution(self, n_recent: int = 40) -> dict[str, float]:
        """Family -> share among recent non-system events (for entropy)."""
        events = [e for e in self.store.list(n_recent, order="desc") if e.visibility != "system"]
        if not events:
            return {}
        counts: dict[str, int] = {}
    ...
    def revisit(self, *, thread_id: str | None = None, query: str | None = None, n: int = 5) -> list[RetrievalHit]:
        ...
        recent_ids = {e.id for e in self._recent_for_recall(10)}
        ...
        if thread_id:
            ...
            hits = hits[:n]
        if query:
            [e for e in self._non_system_pool(300, exclude_ids=recent_ids) if e.text]
    ...
    def memory_age(self, event: Event, now: str | None = None) -> float:
        """Age of an event in seconds (from the store's clock)."""
        return self.index.age_seconds(event, now)
