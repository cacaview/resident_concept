...
class MindLoop:
    ...
    def __init__(
        ...
        *,
        ...
        revisit_context_exclude_self: bool = False,
        ...
        # Round 13 (self_thread_concentration, real-model regime): R13 knob —
        # revisit self-justification exclusion. The 28d real-model battery
        # (DOMINANT_THREAD_ANALYSIS.md) showed the dominant thread re-selecting
        # ITSELF 83-106/106 wakes via revisit/persist: the present context the
        # material rule reads was dominated by the thread's OWN prior thoughts
        # (a self-citation loop compounding after the last user input). When
        # this knob is on, the present-context text used to justify re-selecting
        # a DORMANT thread on a revisit wake excludes the candidate threads' own
        # prior thoughts — the thread must be re-derivable from CURRENT
        # world/question/user context, not from itself. This extends the VA/VC
        # context-exclusion hygiene to the revisit path. Recall payloads (the
        # thread's own memories a revisit legitimately draws on to WRITE) are
        # untouched: the knob changes the JUSTIFICATION (selection) context
        # only. Default False = sealed behavior.
        self.revisit_context_exclude_self = bool(revisit_context_exclude_self)
    ...
    async def wake_once(self, trigger: str = "manual", *, circadian_state: str | None = None) -> dict:
        ...
        thought, refl_reason, thread_id, origin = self._reflect(
    ...
    def _reflect(
        ...
        revival_candidate=None,
    ) -> tuple[Any | None, str, str | None, str]:
        ...
        if not recalled and route in _NEEDS_RECALL:
            return None, "no recalled material to draw on", None, None
        ...
        if out is None:
            return None, "model provider unavailable during reflection; resting", None, None

        text = str(out.get("text") or "").strip()
        if out.get("decision") == "noop" or not text:
            return None, "model produced no durable content", None, None
        ...
        return thought, "persisted", thread_id, origin
        ...
        if route == "continuity" and active:
            ...
            if self.dormant_selection == "material" and len(dormant) > 1:
                ...
                if self.zero_history_bootstrap and any(
                    ...
                    # R13: the dormant thread's own prior thoughts are excluded
                    # from the justification context (knob-gated) — re-selection
                    # must be re-derivable from current world/question/user
                    # material, not from the thread citing itself.
                    sel_thread = self._select_material_thread(
                        dormant, exclude_own_thoughts=True)
            else:
                sel_thread = _most_recent(dormant)
    ...
    def _present_context_text(self, exclude_thread_ids: set[str] | None = None) -> str:
        ...
        ``exclude_thread_ids`` (R13): events linked to these threads are not
        the present context — they are that thread's OWN prior output. Used on
        the revisit path when ``revisit_context_exclude_self`` is on."""
        parts: list[str] = []
        for e in self.store.list(12, order="desc"):
            if e.type.startswith(("wake.", "sleep.", "telemetry.", "circadian")):
                continue
            if exclude_thread_ids and e.links.get("thread_id") in exclude_thread_ids:
                continue
    ...
    def _select_material_thread(self, threads, exclude_own_thoughts: bool = False):
        ...
        ``exclude_own_thoughts`` (R13, revisit path only): the present-context
        read excludes the candidate threads' own prior thoughts (see
        ``revisit_context_exclude_self``) — a dormant thread must match CURRENT
        material, not its own output."""
        eligible = threads
        ...
        if exclude_own_thoughts and self.revisit_context_exclude_self:
            exclude_ids = {t.thread_id for t in threads}
        ctx = self._present_context_text(exclude_thread_ids=exclude_ids)
        if not ctx.strip():
