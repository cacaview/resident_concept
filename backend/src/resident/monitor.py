...
def monitor_timeline(store: EventStore, *, limit: int = 200,
    ...
    return {
        ...
        "counts": {
            "total": store.count(),
            # a genuine no-op wake: wake.completed is written for BOTH the
            # persisted and the noop outcome (Mind._finish_noop), so counting
            # it would report every wake as a noop — count wake.noop instead
...
def _shadow_top_pairs(items) -> list[tuple]:
    """Normalise a shadow top-k entry list to ``(id, score)`` tuples.

    The live writer (``semantic.shadow_compare``) records ``legacy_top`` /
    ``semantic_top`` as bare event-id **strings**; the historical sidecar shape
    (and the tests) use ``{"id", "score"}`` **dicts**. Handle both so this
    read-only view never 500s on the real sidecar (a ``str`` has no ``.get``).
    """
    out = []
    for m in (items or [])[:5]:
        if isinstance(m, dict):
            score = m.get("score")
            out.append((m.get("id"), round(score, 3) if isinstance(score, (int, float)) else None))
        else:
            out.append((m, None))
    return out
        ...
        with open(shadow_log_path, encoding="utf-8") as fh:
            for line in fh:
                ...
                records.append({
                    ...
                    "legacy_top": _shadow_top_pairs(r.get("legacy_top")),
                    "semantic_top": _shadow_top_pairs(r.get("semantic_top")),
