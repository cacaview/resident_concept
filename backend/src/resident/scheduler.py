...
- Out-of-band wakes (e.g. "the user just messaged") are requested via
  ``CircadianScheduler.request_wake``, which pokes the beat loop so a fresh
  message is handled on the next beat rather than waiting out the interval.
- One bad wake never kills the loop: each cycle's exceptions are caught.

Thread-safety choice for ``CircadianScheduler.request_wake``: the scheduler's
``asyncio.Event`` is set via ``call_soon_threadsafe`` on the scheduler's event
loop when one is running (safe from any thread); otherwise the event is set
directly (CPython's GIL makes the set atomic) and is picked up when the loop
starts.
...
class WakeScheduler:
    """Background loop that periodically runs one wake."""
