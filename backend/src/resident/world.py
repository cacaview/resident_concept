...
def _norm_juxtaposition_text(s: object) -> str:
    """R13: normalized text for the rest-route distinctness check — casefold,
    whitespace/punctuation stripped (CJK characters are alnum, so they stay)."""
    if not isinstance(s, str):
        return ""
    return "".join(ch for ch in s.casefold() if ch.isalnum())


def _juxtaposition_distinct(a_text: object, b_text: object, a_id: object, b_id: object) -> bool:
    """R13 rest-route distinctness: two juxtaposed items are distinct iff they
    are DIFFERENT events AND their normalized texts are not identical. A
    structural condition, not a score."""
    if a_id is not None and b_id is not None and str(a_id) == str(b_id):
        return False
    na, nb = _norm_juxtaposition_text(a_text), _norm_juxtaposition_text(b_text)
    if not na or not nb:
        return False
    return na != nb
...
class WorldParams:
    ...
    # --- rest-route association distinctness (R13, round 13) ---------------
    # The 28d real-model battery showed rest/noop-path association thoughts
    # built from two VERBATIM templates fitted to arbitrary material pairs,
    # including degenerate self-pairs (「农业」想起「农业」). When on, the
    # juxtaposed items must be DISTINCT — different event ids AND non-identical
    # normalized text — otherwise the wake stays a silent rest. A structural
    # distinctness condition, not a score threshold. Default False = sealed.
    rest_assoc_require_distinct: bool = False


#: the "disabled" arm (A): no world at all. Kept as a named preset.
WORLD_OFF: WorldParams = WorldParams(enabled_modes=())
...
class WorldWindowEngine:
    ...
    def _maybe_thought(self, item: WorldItem, obs, mode: str) -> tuple[object | None, str | None]:
        ...
        if prior is not None and self.rng.random() < self.params.promotion_prob:
            # R13 rest-route distinctness: the reactivation thought juxtaposes
            # THIS material and the OLD one — if they are not distinct (same
            # observation, or identical normalized title), the template would
            # fire on a degenerate self-pair (「X」想起「X」); a silent rest
            ...
            if self.params.rest_assoc_require_distinct and not _juxtaposition_distinct(
                    item.title, old_title, obs.id, old_obs.id if old_obs else None):
                return None, None
        ...
        if self.rng.random() < self.params.association_prob:
            ...
            if assoc is not None:
                # R13 rest-route distinctness (association arm): same rule —
                # the old memory must be a genuinely distinct item, not the
                # same event or the same normalized text as this material.
                if self.params.rest_assoc_require_distinct and not _juxtaposition_distinct(
                        item.title, assoc.text, obs.id, assoc.id):
                    return None, None
