...
def test_assemble_evidence_local_hour_tracks_host_timezone(tmp_path):
    """The day/night clock must follow the machine's LOCAL wall clock, not the
    UTC reference instant the store is written in: for a given absolute instant,
    `local_hour` equals the UTC hour shifted by the host's UTC offset (mod 24).
    On a UTC host the two coincide; on a UTC+8 host a 12:00 UTC instant reads
    20:00 local. This is a portability guard — it is only non-trivial on a
    non-UTC host (the live Windows box is UTC+8)."""
    now_utc = datetime(2026, 6, 1, 12, 0, 0, tzinfo=timezone.utc)
    ev = CircadianStateMachine.assemble_evidence(
        EventStore(tmp_path / "c.sqlite3"), now_utc.isoformat(), current=ACTIVE)
    local = now_utc.astimezone()
    # hour-of-day rendered in the host-local timezone
    assert ev.local_hour == pytest.approx(local.hour + local.minute / 60.0, abs=1e-6)
    # equivalently: the UTC hour shifted by the host's UTC offset, wrapped to [0,24)
    offset_h = (local.utcoffset() or timedelta(0)) / timedelta(hours=1)
    assert ev.local_hour == pytest.approx((12.0 + offset_h) % 24.0, abs=1e-6)
