...
def test_recent_life_window_returns_latest_visible_events():
    events = [_e(i, "thought.created", content={"text": str(i)}) for i in range(1, 6)]
    ms = MentalState(events)
    assert [e.id for e in ms.recent_life(n=2)] == [events[4].id, events[3].id]
...
def test_from_store_replays_events_after_first_thousand(tmp_path: Path):
    store = EventStore(tmp_path / "long_state.db")
    store.append(EventCreate(type="thread.created", content={"thread_id": "thr_1"}))
    for _ in range(999):
        store.append(EventCreate(type="wake.completed"))
    final = store.append(EventCreate(type="thread.activated", content={"thread_id": "thr_1"}))

    state = MentalState.from_store(store)
    assert state.threads["thr_1"].state == "activated"
    assert state.threads["thr_1"].last_event_id == final.id
