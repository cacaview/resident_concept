...
def _seed_life(clock: Clock) -> EventStore:
    ...
    # the real noop shape from Mind._finish_noop: wake.noop first, then
    # wake.completed (result="noop") citing it — both are one resting wake
    noop = store.append(EventCreate(
        type="wake.noop", visibility="system",
        content={"route": "rest", "reason": "nothing durable yet"},
        provenance={"source": "test"}))
    store.append(EventCreate(
        type="wake.completed", visibility="system",
        content={"route": "rest", "result": "noop"},
        links={"caused_by": [noop.id]}, provenance={"source": "test"}))
...
class TestViews:
    ...
    def test_timeline_noops_counts_only_true_noop_wakes(self):
        """The "noops" counter must count genuine no-op wakes, not all
        completed wakes: ``wake.completed`` is written for BOTH the persisted
        and the noop outcome (Mind._finish_noop appends wake.noop AND
        wake.completed), so counting it inflates the number by every
        successful wake. Seed: 3 real noops + 2 persisted wakes, assert the
        count is exactly the 3 noops (the old implementation returned 5)."""
        import tempfile

        tmp = tempfile.mkdtemp()
        store = EventStore(f"{tmp}/m.sqlite3", now_fn=lambda: T0)
        # 3 genuine no-op wakes — the real shape from Mind._finish_noop:
        # wake.noop first, then wake.completed with result="noop"
        for _ in range(3):
            store.append(EventCreate(
                type="wake.started", visibility="system",
                content={"trigger": "circadian"}, provenance={"source": "test"}))
            n = store.append(EventCreate(
                type="wake.noop", visibility="system",
                content={"route": "rest", "reason": "nothing durable yet"},
                provenance={"source": "test"}))
            store.append(EventCreate(
                type="wake.completed", visibility="system",
                content={"route": "rest", "result": "noop"},
                links={"caused_by": [n.id]}, provenance={"source": "test"}))
        # 2 successful wakes — wake.completed with result="persisted" + the
        # thought it persisted (NO wake.noop event)
        for i in range(2):
            store.append(EventCreate(
                type="wake.started", visibility="system",
                content={"trigger": "user"}, provenance={"source": "test"}))
            t = store.append(EventCreate(
                type="thought.created", visibility="private",
                content={"text": f"persisted thought {i}"},
                provenance={"source": "test"}))
            store.append(EventCreate(
                type="wake.completed", visibility="system",
                content={"route": "personal", "result": "persisted",
                         "event_id": t.id, "recalled": []},
                links={"caused_by": [t.id]}, provenance={"source": "test"}))
        tl = monitor_timeline(store)
        assert tl["counts"]["noops"] == 3        # not 3 + 2 = 5
        assert tl["counts"]["thoughts"] == 2
        assert store.count("wake.completed") == 5, "sanity: 5 completed wakes"
    ...
    def test_retrieval_trace_reads_live_string_sidecar(self, tmp_path):
        """The LIVE writer (semantic.shadow_compare) emits legacy_top/semantic_top
        as bare event-id STRINGS, not {id, score} dicts — the read-only view must
        handle that shape (a str has no .get; this is what 500'd on the live box)."""
        clock = Clock()
        store = _seed_life(clock)
        sidecar = tmp_path / "semantic_shadow.jsonl"
        sidecar.write_text(json.dumps({
            "route": "continuity", "query": "意面",
            "legacy_top": ["evt_a", "evt_b"],
            "semantic_top": ["evt_c"],
            "overlap": 0, "top1_differs": True,
        }), encoding="utf-8")
        rt = monitor_retrieval_trace(store, shadow_log_path=str(sidecar))
        rec = rt["shadow"]["records"][0]
        # bare id strings normalise to (id, None) pairs — no AttributeError
        assert rec["legacy_top"] == [("evt_a", None), ("evt_b", None)]
        assert rec["semantic_top"] == [("evt_c", None)]
        assert rec["top1_differs"] is True
