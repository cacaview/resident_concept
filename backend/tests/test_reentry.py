...
# ------------------------------------------------------------ action re-entry
# ADR-0020: a completed DEED surfaces on re-entry (shareable, strength 2); the
# impulse (intent.*) and the unmet attempt (action.failed) stay private and are
# recalled by the Mind, not pushed to the user.


def test_shareable_completed_action_is_shared(tmp_path):
    s = make_store(tmp_path)
    u = append_chat(s)
    a = s.append(
        EventCreate(
            ...
            visibility="shareable",
            content={"summary": "分析了工作区里的三个文件", "capability": "read_only"},
            links={"caused_by": [u.id]},
        )
    )
    res = ReentryEngine(s).evaluate()
    assert res["decision"] == "share_now"
    c = next(x for x in res["candidates"] if x["event_ids"] == [a.id])
    assert c["type"] == "action.completed"
    assert c["strength"] == 2
    assert "完成了一个行动" in c["claim"]
    # anti-fabrication: the cited deed resolves
    assert ReentryEngine(s).validate_candidate(c) is True


def test_private_action_and_intent_are_not_auto_shared(tmp_path):
    s = make_store(tmp_path)
    append_chat(s)
    s.append(
        EventCreate(
            type="action.failed", visibility="private",
            content={"summary": "未完成", "reason": "timeout"},
        )
    )
    s.append(
        EventCreate(
            type="intent.proposed", visibility="private",
            content={"text": "想做个动作", "capability": "read_only", "source": "mind"},
        )
    )
    res = ReentryEngine(s).evaluate()
    assert res["decision"] == "nothing_to_share"
...
# ------------------------------------- ADR-0022: validation on the production path
# The anti-fabrication guarantee must hold where candidates are PERSISTED
# (run()), not only in the isolated predicate: a candidate citing an event
# that does not exist in the store is recorded as a provenance_failed skip and
# never enters reentry.proposed.


def _phantom_candidate(phantom_id: str, created_at: str = "") -> dict:
    return {
        "claim": "我虚构了一件事",
        "event_ids": [phantom_id],
        "type": "thought.created",
...
def test_run_rejects_candidate_citing_nonexistent_event(tmp_path, monkeypatch):
    """Mixed run: a valid candidate plus one whose provenance does not resolve.
    The valid one is proposed unchanged; the phantom is recorded as a
    provenance_failed skip and absent from the proposed event entirely."""
    s = make_store(tmp_path)
    u = append_chat(s)
    t = s.append(
        EventCreate(
    ...
    eng = ReentryEngine(s)
    phantom = "evt_never_appended"
    orig_evaluate = ReentryEngine.evaluate

    def evaluate_with_phantom(self, since=None):
        res = orig_evaluate(self, since)
        res["candidates"].append(_phantom_candidate(phantom, t.created_at))
        res["decision"] = "share_now"
        return res

    monkeypatch.setattr(ReentryEngine, "evaluate", evaluate_with_phantom)
    res = eng.run()

    # the valid candidate still drives the decision and the proposal
    assert res["decision"] == "share_now"
    assert res["proposed_event_id"]
    proposed = s.get(res["proposed_event_id"])
    assert proposed is not None and proposed.type == "reentry.proposed"
    proposed_ids = [i for c in proposed.content["candidates"] for i in c["event_ids"]]
    assert t.id in proposed_ids
    # the phantom is in NEITHER the content nor the links of the proposal
    assert phantom not in proposed_ids
    assert phantom not in (proposed.links.get("related_to") or [])
    # it is recorded as a rejection, with the failure reason
    skipped = [e for e in s.list(50, type_prefix="reentry.skipped")]
    assert len(skipped) == 1
    assert skipped[0].content["reason"] == "provenance_failed"
    assert phantom in skipped[0].content["missing_event_ids"]
    assert skipped[0].links["caused_by"] == [res["evaluated_event_id"]]
    assert res["rejected_event_ids"] == [skipped[0].id]
    # the evaluated event agrees with what was persisted
    evaluated = s.get(res["evaluated_event_id"])
    assert evaluated.content["decision"] == "share_now"
    assert evaluated.content["candidate_count"] == 1


def test_run_all_candidates_rejected_is_nothing_to_share(tmp_path, monkeypatch):
    """If every candidate fails provenance validation there is nothing to
    propose: the run degrades to the nothing_to_share shape and the skip
    carries the provenance failure reason."""
    s = make_store(tmp_path)
    append_chat(s)
    eng = ReentryEngine(s)
    phantom = "evt_never_appended"
    ...
    def evaluate_phantom_only(self, since=None):
        res = orig_evaluate(self, since)
        assert res["candidates"] == []  # nothing real to share
        res["candidates"] = [_phantom_candidate(phantom)]
        res["decision"] = "share_now"
        return res

    monkeypatch.setattr(ReentryEngine, "evaluate", evaluate_phantom_only)
    res = eng.run()

    assert res["decision"] == "nothing_to_share"
    assert "proposed_event_id" not in res
    assert s.count("reentry.proposed") == 0
    skipped = [e for e in s.list(50, type_prefix="reentry.skipped")]
    assert len(skipped) == 1
    assert skipped[0].content["reason"] == "provenance_failed"
    assert phantom in skipped[0].content["missing_event_ids"]
    assert res["skipped_event_id"] == skipped[0].id
