...
import { PermissionAsksPanel } from "./components/PermissionAsksPanel";
...
function mergeHistory(history: ChatMessage[], current: ChatMessage[]): ChatMessage[] {
  ...
  const local = current.filter((message) => !knownIds.has(message.id));
...
export function App() {
  ...
  useEffect(() => {
    ...
    let loading = false;
    const loadHistory = () => {
      if (online === false || historyLoaded.current || loading) return;
      loading = true;
      void api
        ...
        .then((evts) => {
          ...
          const msgs: ChatMessage[] = evts
            ...
            .filter((m) => m.text !== "");
          setMessages((cur) => mergeHistory(msgs, cur));
        })
        .catch(() => {
          // Retry on the next poll when the event endpoint is available again.
        })
        .finally(() => {
          loading = false;
        });
    ...
    loadHistory();
    const timer = window.setInterval(loadHistory, POLL_MS);
    return () => {
      cancelled = true;
  ...
  async function handleSend() {
    ...
    try {
      ...
      setMessages((m) => uniqueById(m.map((msg) => {
        if (msg.id === userId) return { ...msg, id: r.user_event_id };
        if (msg.id === pendingId) return { ...msg, id: r.resident_event_id, text: r.reply, pending: false };
        return msg;
      })));
    } catch {
      setMessages((m) =>
        m.map((msg) =>
          msg.id === pendingId ? { ...msg, text: "（未收到回复；消息可能已送达，请先查看对话记录再决定是否重发）", pending: false, error: true } : msg,
        ),
      );
    } finally {
      setSending(false);
