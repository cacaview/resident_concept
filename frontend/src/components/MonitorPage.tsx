...
export function MonitorPage({ online }: { online: boolean | null }) {
  ...
  const refresh = useCallback(async () => {
    const [t, th, q, c, r] = await Promise.allSettled([
    ...
    if (th.status === "fulfilled") setThreads(th.value);
    if (q.status === "fulfilled") setQuestions(q.value);
    if (c.status === "fulfilled") setContinuity(c.value);
    if (r.status === "fulfilled") setTrace(r.value);
    const labels = ["生命时间线", "线程", "问题", "缺席与回归", "检索轨迹"];
    setFailedPanels([t, th, q, c, r].flatMap((result, index) => result.status === "rejected" ? [labels[index]] : []));
  }, []);
  ...
  const select = useCallback(
    (id: string) => {
      ...
      setSelected(id);
      setProv(null);
      api.monitorProvenance(id)
        .then((result) => {
          if (selectionVersion.current === version) setProv(result);
        })
        .catch(() => {
          if (selectionVersion.current === version) setProv(null);
        });
    },
    [],
  );
  ...
  return (
    <main className="monitor">
      ...
      {failedPanels.length > 0 && (
        <p className="muted" role="status">以下区域暂未更新，显示上次成功的数据：{failedPanels.join("、")}。</p>
      )}
